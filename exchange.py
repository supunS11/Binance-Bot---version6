...
def _private_rest_call(context, func, *args, **kwargs):
    ...
    try:
        ...
        if "-1021" in message or "timestamp" in message and "recvwindow" in message:
            log_warning(
                f"Binance timestamp drift detected | CALL={context} | resyncing"
            )
            ...
            if sync_client_time():
                try:
                    ...
                    _set_private_rest_backoff(retry_error, context)
                    raise
...
def place_tp_sl(
    ...
    try:
        ...
        if sl_enabled and preserve_existing_sl:
            if sl_price_override is not None:
                existing_sl_info = find_matching_close_position_stop(
                    symbol,
                    side,
                    sl_price_override,
                    position_side=position_side,
                ...
                if existing_sl_info is None:
                    raise RuntimeError(
                        f"{symbol} existing hard stop could not be verified; "
                        "TP reprice blocked"
                    )
            else:
                existing_sl_info = get_open_stop_loss_info(symbol) or {}
