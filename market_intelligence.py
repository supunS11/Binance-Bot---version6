...
class MarketFlowMonitor:
    def __init__(self, symbols, shutdown_event=None):
        ...
        self.data = {
            symbol: {
                ...
                "ofi_samples": 0,
                "last_best_bid": 0.0,
                "last_best_bid_qty": 0.0,
                "last_best_ask": 0.0,
                "last_best_ask_qty": 0.0,
                "depth_samples": 0,
    ...
    def _handle_depth(self, data):
        ...
        with self.lock:
            item = self.data[symbol]
            previous_bid = item["last_best_bid"]
            previous_bid_qty = item["last_best_bid_qty"]
            previous_ask = item["last_best_ask"]
            previous_ask_qty = item["last_best_ask_qty"]

            if previous_bid > 0 and previous_ask > 0:
                if best_bid > previous_bid:
                    bid_change = best_bid_qty
                elif best_bid == previous_bid:
                    bid_change = best_bid_qty - previous_bid_qty
                else:
                    bid_change = -previous_bid_qty

                if best_ask < previous_ask:
                    ask_change = best_ask_qty
                elif best_ask == previous_ask:
                    ask_change = best_ask_qty - previous_ask_qty
                else:
                    ask_change = -previous_ask_qty

                queue_scale = max(
                    best_bid_qty + best_ask_qty +
                    previous_bid_qty + previous_ask_qty,
                    1e-12,
                )
                ofi = _clip((bid_change - ask_change) / queue_scale)
                ofi_alpha = _clip(
                    getattr(config, "MARKET_FLOW_OFI_EMA_ALPHA", 0.2),
                    0.01,
                    1.0,
                )
            ...
            item["last_best_bid"] = best_bid
            item["last_best_bid_qty"] = best_bid_qty
            item["last_best_ask"] = best_ask
            item["last_best_ask_qty"] = best_ask_qty
    ...
    def snapshot(self, symbol):
        ...
        with self.lock:
            ...
            ofi = float(item["ofi"])
            ofi_samples = int(item["ofi_samples"])
