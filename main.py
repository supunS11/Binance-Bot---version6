...
def _manage_adaptive_ladder_dca_position(
    ...
    if (
        ...
        (
            ...
            (
                position_state.get("multi_tp_stage") == TP1_PENDING and
    ...
    fresh_details = get_open_position_details(symbol, force=True)

    if fresh_details is None:
        clear_dca_reservation(state, symbol, dca_level)
        log_warning(
            f"{symbol} DCA aborted | fresh position snapshot unavailable"
        )
        return

    fresh_position = fresh_details.get(symbol)

    if not fresh_position or fresh_position.get("side") != side:
        clear_dca_reservation(state, symbol, dca_level)
        log_warning(
            f"{symbol} DCA aborted | live position vanished or changed side"
        )
        return

    position_detail = fresh_position
    pre_position_amount = float(fresh_position.get("amount", 0) or 0)
    old_quantity = abs(pre_position_amount)
    avg_entry = float(
        fresh_position.get("entry_price") or avg_entry or 0
    )
    order = place_market_order(
        symbol,
        order_side,
    ...
    if not order:
        clear_dca_reservation(state, symbol, dca_level)
        log_warning(f"{symbol} DCA aborted | execution failed with no fill")
        return
    ...
    filled_quantity = get_reconciled_executed_quantity(order)
    ...
    level_info["requested_quantity"] = quantity
    level_info["executed_quantity"] = filled_quantity
    ...
    used_dca_margin = dca_margin * min(filled_quantity / quantity, 1)
    ...
    if config.DCA_REPRICE_TP_AFTER_FILL:
        if cancel_open_protection_orders(symbol):
            ...
            if not protection_ok:
                log_error(
                    f"{symbol} DCA replacement protection was not secured"
                )
                update_position_tp_status(
                    state,
                    symbol,
                ...
                fail_safe_close_unprotected_position(
                    ...
                    position_side=position_detail.get("position_side"),
                    reference_price=current_price,
                    context=f"DCA_LEVEL_{dca_count + 1}_PROTECTION",
                )
                return
        else:
            log_warning(
    ...
    if new_tp_info:
        if not update_position_tp_status(
            ...
            log_error(
                f"{symbol} DCA protection state persistence failed | "
                f"LEVEL={dca_count + 1}"
            )
            fail_safe_close_unprotected_position(
                symbol,
                position_side=position_detail.get("position_side"),
                reference_price=current_price,
                context=f"DCA_LEVEL_{dca_count + 1}_TP_STATE_PERSISTENCE",
            )
            shutdown_event.set()
            return
    ...
    for attempt in range(1, attempts + 1):
        ...
        for detail in position_details:
            ...
            if close_position_market(
                ...
                context="TARGET_MARGIN_STOP",
            ):
                cancel_open_protection_orders(symbol)
                log_warning(f"{symbol} target margin stop close confirmed")
            else:
...
class TargetMarginBalanceMonitor:
    ...
    def stop(self):
        ...
        if (
            ...
            # A target-margin trigger closes and verifies positions on this
            # worker. Cleanup must not flush telemetry or return the process
            # while that risk-reduction workflow is still in flight.
            thread.join()
...
class DcaWebsocketMonitor:
    ...
    def _handle_multi_tp_runner(self, symbol, mark_price, state):
        ...
        if stage == TP1_PENDING:
            if trigger_now and not trigger_seen:
                trigger_seen = self._persist_multi_tp_updates(
                    state,
                    symbol,
                    {
        ...
        if not self._multi_tp_retry_ready(symbol, check_seconds):
            return False
    ...
    def _configure_multi_tp_runner_legacy(
        self,
        symbol,
        mark_price,
        state,
...
def enrich_candidate_market_context(candidate, flow_monitor, breadth_context):
    ...
    if intraday.get("active"):
        route = f"INTRADAY_{intraday.get('route') or 'TREND'}"
    elif (side_data.get("continuation_pullback") or {}).get("active"):
...
def attach_candidate_shadow_order_flow(candidate, shadow_monitor):
    """Attach observation-only analytics after all decision scores are final."""
    ...
    try:
        candidate["shadow_order_flow"] = shadow_monitor.snapshot(
            ...
            context={
                ...
                "route": (candidate.get("market_context") or {}).get(
                    "route"
                ),
...
def execute_entry_candidate(
    ...
    try:
        ...
        if pre_order_details is None:
            log_warning(
                f"{symbol} entry skipped | fresh pre-order snapshot unavailable"
            )
            return position_details, open_positions, False
        ...
        position_state["shadow_order_flow"] = (
            candidate.get("shadow_order_flow") or {}
        )
        ...
        if not upsert_position_state(trade_state, symbol, position_state):
            log_error(
                f"{symbol} durable entry state was not secured; "
                "closing position as a fail-safe"
            )
            fail_safe_close_unprotected_position(
                symbol,
                reference_price=current_price,
                context="ENTRY_STATE_PERSISTENCE",
            )
            return position_details, open_positions, False
        append_signal_journal(
            symbol,
        ...
        open_positions[symbol] = (
            filled_quantity if signal == "BUY" else -filled_quantity
        )
...
def run_bot():
    ...
    if (
        ...
        log_error(
            "BOT START ABORTED | Binance one-way position mode could not be "
            "confirmed. This v6 state model is not safe for simultaneous hedge legs."
        )
        return

    if not validate_execution_telemetry_path():
        log_error(
            "Execution telemetry is unavailable; trading continues with "
            "reconciliation but VPS telemetry must be repaired"
        )
    ...
    try:
        ...
        try:
            ...
            log_warning(
                "Order-flow shadow startup degraded; core bot continues | "
                f"ERROR={exc}"
            )
        ...
        for monitor_name, monitor in (
            ("target margin", target_margin_monitor),
            ("market flow", flow_monitor),
            ("order-flow shadow", shadow_flow_monitor),
            ("DCA websocket", dca_monitor),
        ):
            if monitor is None:
                continue
            ...
            try:
                ...
                log_warning(f"{monitor_name} monitor stop warning: {exc}")
