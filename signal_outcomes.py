...
def _state_path():
    return _resolve_path(
        getattr(
            ...
            "data/signal_outcomes_pending_v6.json",
        )
    )
...
def _route(candidate):
    market_route = str(
        (candidate.get("market_context") or {}).get("route") or ""
    ).strip().upper()
...
def observe_signal_outcomes(symbol, close_price, high_price=None, low_price=None):
    ...
    calibration_target = max(
        float(getattr(config, "SIGNAL_CALIBRATION_HORIZON_HOURS", 4)),
        0.01,
    )
    calibration_horizon = next(
        (horizon for horizon in horizons if horizon >= calibration_target),
        horizons[-1],
    )
        ...
        with _lock:
            ...
            for signal_id, item in list(pending.items()):
                ...
                for horizon in horizons:
                    ...
                    if (
                        horizon == calibration_horizon and
                        not item.get("calibration_recorded")
                    ):
                        threshold = float(
                            getattr(
                                config,
                        ...
                        item["calibration_recorded"] = True
