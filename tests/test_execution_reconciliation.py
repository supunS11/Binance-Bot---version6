...
class ExchangeRuleTests(OfflineExecutionCase):
    ...
    def test_shadow_depth_budget_preserves_core_weight_reserve(self):
        self._set_config("BINANCE_PUBLIC_WEIGHT_LIMIT_PER_MINUTE", 700)
        self._set_config("BINANCE_PUBLIC_RATE_WINDOW_SECONDS", 60)
        self._set_config("ORDER_FLOW_SHADOW_CORE_WEIGHT_RESERVE", 500)

        with exchange._public_request_lock:
            exchange._public_request_weights.clear()

        self.addCleanup(exchange._public_request_weights.clear)
        accepted = [
            exchange._try_reserve_shadow_public_weight(20)
            for _ in range(11)
        ]

        self.assertEqual(accepted[:10], [True] * 10)
        self.assertFalse(accepted[10])

        with exchange._public_request_lock:
            used = sum(
                weight for _, weight in exchange._public_request_weights
            )
...
class ShadowAttachmentIsolationTests(unittest.TestCase):
    def test_shadow_attachment_cannot_change_rank_or_market_context(self):
        candidate = {
            "symbol": SYMBOL,
            "signal": "BUY",
            "rank_score": 17.25,
            "market_context": {
                "flow": {"available": True, "buy_score": 3},
                "breadth": {"available": True, "buy_score": 2},
                "route": "TREND",
            },
        }
        original_rank = candidate["rank_score"]
        original_context = copy.deepcopy(candidate["market_context"])
        monitor = Mock()
        ...
        result = main.attach_candidate_shadow_order_flow(candidate, monitor)

        monitor.snapshot.assert_called_once_with(
            SYMBOL,
            context={
                "signal_side": "BUY",
                "route": "TREND",
        ...
        self.assertEqual(candidate["rank_score"], original_rank)
        self.assertEqual(candidate["market_context"], original_context)
        self.assertEqual(candidate["shadow_order_flow"]["shadow_score"], -5)
